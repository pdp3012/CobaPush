...
class BlibliScraper:
    def __init__(self):
        """
        Inisialisasi scraper dengan headers yang mirip browser dan cookies
        """
        self.headers = {
        ...
        # Cookies untuk Blibli
        self.cookies = {
            'Blibli-Device-Id': 'U.c9648d61-a879-459d-8eda-803deec7032c',
            'Blibli-Session-Id': 'e7b8bca5-f59f-4187-a682-7f0f7d0dbab6',
            'cf_clearance': 'Rima6V9p6gHYuAJpoi7l68twPTTWsTWg_F8ohDD6pEU-1754333016-1.2.1.1-1SU4a3.dteukdiW6mLZDpde_7zz2N5TadLWr56laXk4VnEFRIk5sYdxsGG.99FPlC4X2SM0nj7KOeLJMU.rsLK6l88y8Ii60XzcpRKjI6RB3SenwMHexFxezJWuzRm2uLt5UkYQ4jZz7NoqSLP4ky8YT5KN35h7PhUzzovroOmGMgpf4iKznUVhLtYMcH_fIWHw5Xw3Q5eOQRG.QWp3UZrpQrZbXJQXbxy4gFFebMC4',
            'forterToken': 'afebe347cc024253bc784c90453fb7dc_1754333016162_8070_UDAD43b-mnts-a9-r8-n4_25ck_',
        }
    ...
    def get_page_content(self, url, max_retries=3):
        """
        Mendapatkan konten halaman dengan retry mechanism
        """
    ...
    def extract_product_data(self, soup):
        ...
        # Cari container produk dengan CSS selector yang sesuai
        product_containers = soup.find_all('div', class_='els-product')
        ...
        for idx, container in enumerate(product_containers):
            try:
                product_data = self.extract_single_product(container, idx + 1)
    ...
    def extract_single_product(self, element, product_number):
        """
        Ekstrak data produk tunggal dari container
        """
        product_data = {
            'nama_produk': '',
            'harga': '',
    ...
    def extract_product_name(self, element):
        """Ekstrak nama produk dari Blibli"""
        try:
            # Cari berdasarkan CSS selector yang diberikan
            title_wrapper = element.find('div', class_='els-product__title-wrapper')
            if title_wrapper:
                title_span = title_wrapper.find('span', class_='els-product__title')
                if title_span:
                    # Ambil teks dari span yang berisi nama produk
                    spans = title_span.find_all('span')
                    for span in spans:
                        text = span.get_text(strip=True)
                        if text and not text.startswith('Pre-order') and len(text) > 5:
                            return text
                    
                    # Jika tidak ada span yang cocok, ambil teks langsung
                    text = title_span.get_text(strip=True)
                    if text and len(text) > 5:
                        return text
            
            # Fallback: cari berdasarkan title attribute
            title_wrapper = element.find('div', class_='els-product__title-wrapper')
            if title_wrapper:
                title_span = title_wrapper.find('span', class_='els-product__title')
                if title_span and title_span.get('title'):
                    return title_span.get('title')
                    
        except Exception as e:
            print(f"Error ekstrak nama produk: {e}")
            
        return "Nama tidak ditemukan"

    def extract_price(self, element):
        """Ekstrak harga dari Blibli"""
        try:
            # Cari berdasarkan CSS selector yang diberikan
            price_div = element.find('div', class_='els-product__fixed-price')
            if price_div:
                # Ambil label dan nilai harga
                price_label = price_div.find('span', class_='els-product__fixed-price-label')
                price_value = price_div.find_all('span')
                
                if price_label and len(price_value) > 1:
                    label = price_label.get_text(strip=True)
                    value = price_value[1].get_text(strip=True)
                    return f"{label} {value}"
                elif price_div.get('title'):
                    return f"Rp {price_div.get('title')}"
                    
        except Exception as e:
            print(f"Error ekstrak harga: {e}")
            
        return "Harga tidak ditemukan"

    def extract_rating(self, element):
        """Ekstrak rating produk dari Blibli"""
        try:
            # Cari berdasarkan CSS selector yang diberikan
            rating_wrapper = element.find('div', class_='els-product__rating-wrapper')
            if rating_wrapper:
                # Cari teks yang mengandung rating (biasanya format 4.5 atau 4,5)
                rating_text = rating_wrapper.get_text(strip=True)
                rating_match = re.search(r'([0-9]+[.,]?[0-9]*)', rating_text)
                if rating_match:
                    return rating_match.group(1)
                    
        except Exception as e:
            print(f"Error ekstrak rating: {e}")
            
        return ""

    def extract_sold_count(self, element):
        """Ekstrak jumlah terjual dari Blibli"""
        try:
            # Cari berdasarkan CSS selector yang diberikan
            sold_div = element.find('div', class_='els-product__sold')
            if sold_div:
                sold_text = sold_div.get_text(strip=True)
                # Ekstrak angka dari teks "Terjual 590"
                sold_match = re.search(r'Terjual\s*(\d+)', sold_text)
                if sold_match:
                    return f"{sold_match.group(1)} terjual"
                else:
                    return sold_text
                    
        except Exception as e:
            print(f"Error ekstrak jumlah terjual: {e}")
            
        return ""

    def extract_shop_name(self, element):
        """Ekstrak nama toko dari Blibli"""
        try:
            # Cari berdasarkan CSS selector yang diberikan
            shop_span = element.find('span', class_='els-product__seller-name')
            if shop_span:
                return shop_span.get_text(strip=True)
                
        except Exception as e:
            print(f"Error ekstrak nama toko: {e}")
            
        return ""

    def extract_product_link(self, element):
        """Ekstrak link produk dari Blibli"""
        try:
            ...
            for link in links:
                ...
                if href and ('/p/' in href or '/product/' in href):
                    if href.startswith('/'):
                        return 'https://www.blibli.com' + href
                    else:
                        return href
                        
        except Exception as e:
            print(f"Error ekstrak link produk: {e}")
    ...
    def scrape_products(self, keyword, max_pages=3, delay_range=(3, 7)):
        """
        Scraping produk dari multiple halaman dengan delay yang aman
        """
        all_products = []
...
def main():
    ...
    """
    print("="*80)
    print("🤖 BLIBLI SCRAPER - Versi 1.0")
    print("👨‍💻 Dikembangkan berdasarkan referensi Tokopedia Scraper")
