...
def clean_dataframe(df):
    ...
    # Regex untuk memeriksa apakah string mirip tanggal (format umum)
    ...
    # Fungsi helper untuk memeriksa apakah teks adalah konten artikel yang valid
    def is_valid_content(text):
        """Memeriksa apakah teks adalah konten artikel yang valid"""
        if not text or len(str(text).strip()) < 10:
            return False
        # Jika teks adalah URL, bukan konten
        if url_pattern.match(str(text).strip()):
            return False
        # Jika teks terlalu pendek dan mirip tanggal, bukan konten
        if len(str(text).strip()) < 50 and date_like_pattern.search(str(text)):
            return False
        return True
    ...
    for index, row in df.iterrows():
        try:
            ...
            original_values = {
                'judul_berita': str(row.get('judul_berita', '')).strip(),
                'link_berita': str(row.get('link_berita', '')).strip(),
                'tanggal_rilis': str(row.get('tanggal_rilis', '')).strip(),
                'detail_konten': str(row.get('detail_konten', '')).strip()
            }
            
            # --- LANGKAH 1: Periksa dan perbaiki kolom link_berita ---
            if 'link_berita' in df.columns:
                current_link = original_values['link_berita']
                if not url_pattern.match(current_link) and current_link:
                    print(f"    ⚠️  Kolom 'link_berita' bukan link: '{current_link[:50]}...'")
                    
                    # a. Gabungkan isi link_berita ke judul_berita
                    current_title = original_values['judul_berita']
                    new_title = f"{current_title} {current_link}".strip()
                    df.at[index, 'judul_berita'] = new_title
                    print(f"       ✅ Judul diperbarui: '{new_title[:50]}...'")
                    
                    # b. Geser data dari tanggal_rilis ke link_berita
                    current_date = original_values['tanggal_rilis']
                    df.at[index, 'link_berita'] = current_date
                    print(f"       ✅ link_berita diisi dengan data dari tanggal_rilis: '{current_date[:50]}...'")
                    
                    # c. Geser data dari detail_konten ke tanggal_rilis
                    current_content = original_values['detail_konten']
                    df.at[index, 'tanggal_rilis'] = current_content
                    print(f"       ✅ tanggal_rilis diisi dengan data dari detail_konten: '{current_content[:50]}...'")
                    
                    # d. Kosongkan detail_konten
                    df.at[index, 'detail_konten'] = ""
                    print(f"       ✅ detail_konten dikosongkan")
            
            # --- LANGKAH 2: Periksa dan perbaiki kolom tanggal_rilis ---
            if 'tanggal_rilis' in df.columns:
                current_date = str(df.at[index, 'tanggal_rilis']).strip()
                if url_pattern.match(current_date) and current_date:
                    print(f"    ⚠️  Kolom 'tanggal_rilis' berisi link: '{current_date[:50]}...'")
                    
                    # a. Geser data dari detail_konten ke tanggal_rilis
                    current_content = str(df.at[index, 'detail_konten']).strip()
                    df.at[index, 'tanggal_rilis'] = current_content
                    print(f"       ✅ tanggal_rilis diisi dengan data dari detail_konten: '{current_content[:50]}...'")
            ...
            # --- LANGKAH 3: Periksa dan perbaiki kolom detail_konten ---
            if 'detail_konten' in df.columns:
                current_content = str(df.at[index, 'detail_konten']).strip()
                
                # Jika detail_konten berisi tanggal atau bukan konten yang valid
                if not is_valid_content(current_content) and current_content:
                    print(f"    ⚠️  Kolom 'detail_konten' berisi data tidak valid: '{current_content[:50]}...'")
                    
                    # Kosongkan detail_konten
                    df.at[index, 'detail_konten'] = ""
                    print(f"       ✅ detail_konten dikosongkan")
            
            print(f"    ✅ Baris {index} selesai diproses")
